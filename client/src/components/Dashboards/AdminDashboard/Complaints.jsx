...
function Complaints() {
  const getComplaints = async () => {
    ...
    if (!hostel) {
      console.error("No hostel ID found in localStorage");
      return;
    }
    ...
    if (data.success) {
      const complaints = data.complaints.map((complaint) => {
        ...
        return {
          ...
          student: complaint.student ? complaint.student.name : "Unknown",
          room: complaint.student ? complaint.student.room_no : "Unknown",
  ...
  useEffect(() => {
    ...
    setGraphData(
      labels.map(
        (date) => allComplaints.filter((complaint) => complaint.date === date).length
      )
    );
